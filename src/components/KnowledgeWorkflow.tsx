...
export const KnowledgeWorkflow: React.FC = () => {
  ...
  return (
    <section className="py-16 bg-[#041618] border-b border-[#0E353C] relative overflow-hidden" id="knowledge-workflow">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10">
          ...
          <h2 className="text-2xl sm:text-4xl font-sans font-extrabold text-[#F8FAFC] uppercase tracking-tight">
            FROM IDEAS TO <span className="text-emerald-400">SYSTEMS</span>
          </h2>
          <p className="text-xs sm:text-sm font-sans text-[#94A3B8] mt-2">
        ...
        <div className="bg-[#061D20] p-6 rounded-xl border border-[#0E353C] overflow-x-auto">
          <div className="flex items-center min-w-[800px] justify-between">
            {pipelineSteps.map((step, idx) => (
              <React.Fragment key={step.title}>
                <div className="flex flex-col items-center text-center p-3 rounded-lg bg-[#031214] border border-[#0E353C] hover:border-emerald-500/40 hover:bg-[#082226] transition-all w-28">
                  <span className="text-[9px] font-mono text-emerald-400 font-bold mb-1">0{idx + 1}</span>
                  <span className="text-xs font-mono font-bold text-[#F8FAFC] uppercase tracking-wider mb-1">
                    {step.title}
                  </span>
